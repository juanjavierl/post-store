...
from core.catalog.forms import *
...
def optenerProducto(request, id_producto):
    ...
    if request.method == 'POST':
        ...
        datos['precio_uni'] = float(p.pvp)
        datos['total'] = float(int(request.POST['cantidad']) * float(p.pvp))
        if len(data_cli) == 0:
            ...
            dic = {
                ...
                'precio_uni':float(p.pvp),
                'total':float(int(request.POST['cantidad']) * float(p.pvp)),
            ...
            for indice in range(len(data_cli)):   
                if data_cli[indice]['id_producto'] == datos['id_producto']:
                    ...
                    data_cli[indice]['total'] = float(int(data_cli[indice]['cantidad']) * float(p.pvp))
